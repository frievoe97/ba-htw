...
def create_lineplot(ax, data, title, ylabel, hue_order, palette):
    """
    ...
    ax.legend(title='algorithm_value', bbox_to_anchor=(1, 1), loc='upper left')
...
def plot_algorithm(data, algorithm, ax, title):
    """
...
def main():
    ...
    if df is not None:
        ...
        create_lineplot(axes[0], knn_euclidean_data, 'KNN (euclidean)', 'Correct Percent', None, palette)
        create_lineplot(axes[1], knn_sorensen_data, 'KNN (sorensen)', 'Correct Percent', None, palette)
        create_lineplot(axes[2], rf_data, 'Random Forest', 'Correct Percent', None, palette)
        create_lineplot(axes[3], svm_linear_data, 'SVM (linear)', 'Correct Percent', None, palette)
        create_lineplot(axes[4], svm_rbf_data, 'SVM (rbf)', 'Correct Percent', None, palette)

        # X-axis labels for all subplots
        for ax in axes:
        ...
        # Calculate the average and weighted average correct percent for each algorithm and algorithm value
        ...
        # Plots for each algorithm
        plot_algorithm(avg_correct_percent, 'knn_euclidean', axes[0], 'KNN (euclidean)')
        plot_algorithm(avg_correct_percent, 'knn_sorensen', axes[1], 'KNN (sorensen)')
        plot_algorithm(avg_correct_percent, 'random_forest', axes[2], 'Random Forest')
        plot_algorithm(avg_correct_percent, 'svm_linear', axes[3], 'SVM (linear)')
        plot_algorithm(avg_correct_percent, 'svm_rbf', axes[4], 'SVM (rbf)')
